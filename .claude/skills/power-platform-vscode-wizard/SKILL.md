...
- Validate the primary entry-point logical name against reused standard tables, planned custom tables, or current retrofit inventory. Give every planned view an explicit `views.json` disposition. Default to `create-custom`; use `adopt-generated-active` only when the current run can prove table creation and exact unmanaged Active/default generated-view identity. Reused, standard, shared, or preexisting tables use `verify-existing-saved-query` and are never adoption targets. Require authenticated saved-query resolution before app assembly.
