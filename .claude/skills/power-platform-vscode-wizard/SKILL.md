...
- Use only `.wizard-metrics/artifacts/manifest/<normalized-scenario-slug>/` for provenance. Validate scenario, solution, and publisher identity on every manifest read or write; any mismatch stops the stage. Preview cannot create or alter provenance. The legacy global manifest is never trusted automatically and may be copied only through the explicit validated migration helper documented in `docs/wizard-hardening-runbook.md`.
