...
v0.4.0  2026-08-30  Replaced broad publishing with payload-table-scoped PublishXml.
...
if (-not (Test-Path $dataverseRuntimeHelper)) {
  throw "Missing helper script: $dataverseRuntimeHelper"
}
...
$script:WizardManagedFormMarker = 'Generated by 60-build-forms-views.ps1'
...
function Get-TableAttributesMetadata {
  ...
  $results = New-Object System.Collections.Generic.List[object]
  $response = Invoke-Dv -Method 'Get' -Path "EntityDefinitions(LogicalName='$TableLogical')/Attributes?`$select=LogicalName,AttributeType,IsPrimaryId,IsPrimaryName,IsValidForForm,IsValidForRead,DisplayName"
...
function Get-TargetMainForm {
  ...
  $preferred = @($ExistingForms | Where-Object { $_.name -eq $PreferredFormName })
  if ($preferred.Count -gt 1) { throw "Expected at most one Main form named '$PreferredFormName'; found $($preferred.Count)." }
  ...
  $alternate = @($ExistingForms | Where-Object { $_.name -eq $alternateName })
  if ($alternate.Count -gt 1) { throw "Expected at most one Main form named '$alternateName'; found $($alternate.Count)." }
  if ($alternate.Count -eq 1) { return $alternate[0] }
...
function Invoke-WizardFormsViewsBuild {
  ...
  foreach ($table in $tables) {
    ...
    try {
      ...
      $existingForms = @((Invoke-Dv -Method 'Get' -Path "systemforms?`$select=formid,name,type,formxml,description&`$filter=objecttypecode eq '$logical' and type eq 2").value)
      ...
      $formAction = 'skipped'
      $formOwnershipMarker = "$script:WizardManagedFormMarker; scenario=$($scenarioContext.ScenarioSlug)"

      if ($null -ne $targetForm -and $resolvedFormStrategy -eq 'create-new-forms' -and "$(Get-PropertyValue -Object $targetForm -Name 'description')" -cne $formOwnershipMarker) {
        throw "Cannot update form '$resolvedFormName': same-name form is not owned by scenario '$($scenarioContext.ScenarioSlug)'."
      }
      ...
      if ($null -eq $targetForm) {
        ...
        $formBody = @{
          ...
          type           = 2
          description    = $formOwnershipMarker
    ...
    try {
      ...
      $sameNameViews = @($existingViews | Where-Object { $_.name -eq $viewName })
      if ($sameNameViews.Count -gt 1) { throw "Expected at most one public view named '$viewName' for '$logical'; found $($sameNameViews.Count)." }
      $targetView = if ($sameNameViews.Count -eq 1) { $sameNameViews[0] } else { $null }
      ...
      if ($null -eq $targetView) {
        ...
        if ($disposition -eq 'adopt-generated-active' -or (Test-IsWizardManagedView -View $targetView -PrimaryField $primary)) {
          ...
          throw "Cannot update view '$viewName': same-name view is not owned by scenario '$($scenarioContext.ScenarioSlug)'."
        }
      }
  ...
  if ($PreviewOnly) {
    ...
    Write-Host '  Publishing scenario table customizations...' -NoNewline
    try {
      $publishXml = New-WizardEntityPublishXml -EntityLogicalNames @($tables | ForEach-Object { $_.LogicalName })
