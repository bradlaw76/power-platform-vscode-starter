...
The canonical, multi-workload `power-platform-vscode-starter` repository for
building Power Platform solutions from VS Code using the Power Platform CLI and
...
| Workload | Entry point | Use when |
| --- | --- | --- |
| Model-driven app development | `/power-platform-wizard-init` | You need to plan and build or retrofit a model-driven Power App or Dynamics 365 extension. |
| Reporting for an existing model-driven app | `/dataverse-report-wizard` | The app and tables already exist and you want guided native Dataverse charts, dashboards, and FetchXML previews. |
| Direct reporting runtime | `pwsh .\scripts\bootstrap\08-report-wizard.ps1` | You want to run the report wizard directly from PowerShell. |

The [Dataverse Report Wizard guide](docs/dataverse-report-wizard.md) covers
simulation, live metadata discovery, review, and approved deployment. The
reporting workload reuses this starter's authentication, Dataverse runtime,
solution, app-module, publishing, schema, and CI capabilities; it is not a
...
When the folder opens in VS Code, open Copilot Chat and choose the entry point
for your workload. For model-driven app development, enter:
...
For reporting in an existing model-driven app, enter:

```text
/dataverse-report-wizard
```

See the [complete Dataverse Report Wizard guide](docs/dataverse-report-wizard.md)
before live deployment. The report wizard defaults to safe simulation and
requires separate approval before using its deployment mode.
...
- Add native Dataverse charts and dashboards to an existing model-driven app
...
publishing, schema-validation, and CI foundations.
...
| A maker adding reports to an existing model-driven app | A focused wizard for safe preview and approved native report deployment |
...
For model-driven app development, the primary entry point is the VS Code shared
skill. Direct app-wizard routes remain supported and lead to the same
...
For reporting against an existing model-driven app, use
`/dataverse-report-wizard` or
`pwsh .\scripts\bootstrap\08-report-wizard.ps1`. That workload follows:

```text
Report request -> Metadata discovery -> FetchXML preview -> Review -> Approved deployment -> Publish
```

See the [Dataverse Report Wizard guide](docs/dataverse-report-wizard.md) for the
complete reporting workflow, safety gates, generated artifacts, and CI usage.
...
| `08-report-wizard.ps1` | Design and preview native Dataverse reporting for an existing model-driven app; deploy only with explicit approval | Only with `-Deploy` | Yes (`-Force` is required to replace generated local artifacts) |
...
| Document | Purpose |
| --- | --- |
| [docs/dataverse-report-wizard.md](docs/dataverse-report-wizard.md) | Complete guide for reporting against an existing model-driven app, including simulation, preview, deployment approval, and validation |
